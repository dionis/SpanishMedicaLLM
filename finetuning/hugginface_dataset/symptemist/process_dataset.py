...
FILE_PATH = "SympTemist.zip"

path = Path(__file__).parent.absolute()

from urllib import request
URL = 'https://zenodo.org/api/records/8223654/files-archive'

FILE_ZIP =  str(path) + os.sep + "SympTemist.zip"
FILE_ZIP_EXTRAC =  str(path) + os.sep + "SympTemist"

if not os.path.exists( FILE_ZIP ):
   response = request.urlretrieve(URL,  str(path) + os.sep + "SympTemist.zip")
